...
let
  ...
  oakDisplayLayout = pkgs.writeShellScriptBin "oak-display-layout" ''
    ...
    if [ -n "$benq" ] && [ -n "$center" ] && [ -n "$side" ]; then
      args=(
        ...
        --output "$side" --on --mode 1600x900@60.000000Hz --pos 3640,0 --transform 270 --scale 1.01
      )
  ...
  config = ''
    ...
    monitorrule=model:DELL E2020H,width:1600,height:900,refresh:60,x:3640,y:0,scale:1.01,rr:3

...

{
  ...
  doomModules,
  # nix-doom-emacs-unstraightened source tree for build helpers and patches.
  unstraightenedSource,
...
let
  ...
  doomEmacsPackages = lib.foldl' (p: p.overrideScope) (emacsPackagesFor emacs) (
    [
      (
        eself: esuper:
        callPackages (unstraightenedSource + "/elisp-packages-early.nix") { inherit esuper eself; }
      )
      (
        eself: esuper:
        let
          ...
          makePackage =
            ...
            let
              ...
              epkg =
                if hasOrigEPkg then
                  ...
                  (esuper.melpaBuild {
                    ...
                    inherit files;
                    packageRequires = map (dep: eself.${dep}) (generatedPackageRequires.${name} or [ ]);
                  });
              ...
              src =
                ...
                else if experimentalFetchTree then
      ...
      (
        eself: esuper:
        callPackages (unstraightenedSource + "/elisp-packages-late.nix") { inherit esuper eself; }
      )
    ]
